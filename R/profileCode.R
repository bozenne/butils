...
#' @param type the function used for profiling: can be \code{"Rprof"} or \code{"profvis"}.
#' @param plot should the result of the profiling be graphically displayed. 
#' @param ... additional arguments to be passed to the profiling function
#' 
#' @details 
...
#' estimate(m,d)
#' }
#' profileCode(FUN, type = "Rprof")
#' profileCode(FUN, type = "profvis")
#' profileCode(FUN, type = "profvis", plot = TRUE)
#' }
#' 
#' @keywords function profile
#' @export
profileCode <- function(FUN, filename = NULL, type = "Rprof", plot = FALSE, ...){
  if(is.null(filename)){
    filename <- tempfile()
  ...
  }else if(type == "profvis"){
    sProfile <- profvis::profvis(FUN(), ...)
    if(plot){print(sProfile)}
  }
